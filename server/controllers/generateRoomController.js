...
const generateMain = (data) => {
	...
	for (let i = 0; i < options.size; i++) {
		...
		const resolvedSchedule = resolveCollisions(sln);
		population.push(resolvedSchedule);
	}
...
const seed = function () {
	...
	for (let batchCode in data.batchSizeMap) {
		...
		for (let subjectId of subjects) {
			...
			const class_resolved = resolveCollisions(classObj);

			solution.push(class_resolved);
			...
			if (hasLab) {
				...
				const lab_resolved = resolveCollisions(labObj);

				solution.push(lab_resolved);
			}
		}
	}
...
const detectCollisions = (schedule) => {
	const conflicts = [];
	...
	for (let i = 0; i < schedule.length; i++) {
		for (let j = i + 1; j < schedule.length; j++) {
			...
			if (class1.dayOfWeek === class2.dayOfWeek) {
				if (
					class1.staffno === class2.staffno ||
					class1.roomNum === class2.roomNum ||
					class1.batchCode === class2.batchCode
				) {
					if (
						isOverlapping(
							class1.startTime,
...
const resolveCollisions = (schedule) => {
	let conflicts = detectCollisions(schedule);

	while (conflicts.length > 0) {
		conflicts.forEach(({ class1, class2 }) => {
			let adjustedClass = class2;
			...
			for (let time of availableTimes) {
				...
				const newEndTime = new Date(
					new Date(`1970-01-01T${newStartTime}Z`).getTime() +
						2 * 60 * 60 * 1000,
				)
					.toISOString()
					.substr(11, 8);
				...
				if (
					...
					adjustedClass.startTime = newStartTime;
					adjustedClass.endTime = newEndTime;
					break;
				}
			}
		});

		conflicts = detectCollisions(schedule);
	}
...
const localSearch = function (solution, data) {
	for (let i = 0; i < solution.length; i++) {
		for (let j = i + 1; j < solution.length; j++) {
			...
			if (sameDay && sameTime) {
				if (sameRoom || sameLecturer || sameBatch) {
					let foundSlot = false;

					// Try to find a new time slot on the same day
					for (let hour = 8; hour <= 16; hour++) {
						const newStartTime = `${hour}:00:00`;
						const classDuration = calculateDuration(solution[i]);
					...
					if (!foundSlot) {
						const preferredDays = data.lecturerDays[solution[i].staffno];
